...
namespace Sandswept.Survivors.Ranger.SkillDefs.Passive
{
    public class OverchargedProtection : SkillBase<OverchargedProtection>
    {
        ...
        public override string Description => "Hold up to " + Projectiles.DirectCurrent.maxCharge + " $rcCharge$ec. $rcCharge$ec increases $shbase health regeneration$se by up to $sh2.5 hp/s$se and $sharmor$se by up to $sh15$se. $rcCharge decays over time$ec.".AutoFormat();
