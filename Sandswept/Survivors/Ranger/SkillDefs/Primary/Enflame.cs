...
namespace Sandswept.Survivors.Ranger.SkillDefs.Primary
{
    public class Enflame : SkillBase<Enflame>
    {
        ...
        public override string Description => "$suAgile$se. Fire a rapid stream of bullets for $sd80% damage$se. $srHeat$se increases $sdfire rate $se and $sdignite chance$se but $srreduces range$se.".AutoFormat();
