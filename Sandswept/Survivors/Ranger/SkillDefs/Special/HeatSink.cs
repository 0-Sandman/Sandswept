...
namespace Sandswept.Survivors.Ranger.SkillDefs.Special
{
    public class HeatSink : SkillBase<HeatSink>
    {
        ...
        public override string Description => "$suAgile$se. $sdIgnite$se. Release a $sdfire nova$se around you that deals $sd300%$se damage, increasing up to $sd900%$se in $srfull heat$se. $suConsume all$se $srheat$se, gaining an $sdattack speed$se boost, and $srexit overdrive$se.".AutoFormat();
