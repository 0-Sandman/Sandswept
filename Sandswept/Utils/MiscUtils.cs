...
namespace Sandswept.Utils
{
    public static class MiscUtils
    {
        ...
        {
            // for itself
            Iterate(gameObject, primaryColor, emissionAndLightColor, fuckRamps);
            // and for all children recursively FUCK OYUY U YOU YU@OQ!RUEII!I@$!@@(PIOJFSKDIPJDISFSD WRHAMFRAHHAHAHAHJASFMKDJLDSMJKLKI IM SO DONE
            foreach (Transform child in gameObject.transform.GetComponentsInChildren<Transform>())
            {
                Iterate(child.gameObject, primaryColor, emissionAndLightColor, fuckRamps);
            }
        }

        public static void RescaleGameObject(GameObject gameObject, float scaleMultiplier)
        {
            foreach (Transform child in gameObject.transform.GetComponentsInChildren<Transform>())
            {
        ...
        private static void Iterate(GameObject gameObject, Color32 primaryColor, Color32 emissionAndLightColor, bool fuckRamps)
        {
            foreach (ParticleSystem faggot in gameObject.GetComponents<ParticleSystem>())
            {
                var faggotMain = faggot.main;
                var faggotStartColor = faggotMain.startColor;
                faggotStartColor.mode = ParticleSystemGradientMode.Color;
                faggotStartColor.color = primaryColor;

                var faggotColorOverLifetime = faggot.colorOverLifetime;
                if (faggotColorOverLifetime.color.gradient != null && faggotColorOverLifetime.color.gradient.colorKeys.Length > 0)
                {
                    for (int i = 0; i < faggotColorOverLifetime.color.gradient.colorKeys.Length; i++)
                    {
                        var faggotColorKey = faggotColorOverLifetime.color.gradient.colorKeys[i];
                        faggotColorKey.color = Color.white;
                    }
                }
            }
