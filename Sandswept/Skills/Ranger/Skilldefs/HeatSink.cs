...
namespace Sandswept.Skills.Ranger.Skilldefs
{
    public class HeatSink : SkillBase<HeatSink>
    {
        ...
        public override string Description => "$suAgile$se. Release a $sdfire nova$se around you that ignites and deals $sd250%$se base damage, increasing up to $sd1000%$se in full heat. $srConsumes all heat$se, $srdamages you$se for $sr25% max health$se and $srexits overdrive$se.".AutoFormat();
