...
namespace Sandswept.Items.Reds
{
    ...
    public class CeremonialJar : ItemBase<CeremonialJar>
    {
        ...
        public override string ItemFullDescription => $"$sdLink$se enemies on hit. Linking $sd{linkedEnemiesRequirement}$se enemies deals $sd{baseDamage * 100f}%$se $ss(+{stackDamage}% per stack)$se $sdbase damage$se to each. Enemies become immune to getting $sdlinked$se for $sd{linkedEnemyCooldown}$se seconds afterward.".AutoFormat();
