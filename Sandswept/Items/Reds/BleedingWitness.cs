...
namespace Sandswept.Items.Reds
{
    ...
    internal class BleedingWitness : ItemBase<BleedingWitness>
    {
        ...
        public override string ItemFullDescription => $"$sd{hemorrhageChance}%$se chance to $sdhemorrhage$se enemies for $sd{hemorrhageDamage * 100f}%$se base damage. Your $sddamage over time effects$se $shheal$se all allies for $sh{baseDoTHealing * 100f}%$se $ss(+{stackDoTHealing * 100f}% per stack)$se of their $shmaximum health$se.".AutoFormat();
