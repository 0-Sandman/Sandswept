...
namespace Sandswept.Items.Whites
{
    ...
    public class AmberKnife : ItemBase<AmberKnife>
    {
        ...
        public override string ItemFullDescription => ("Gain a $sd" + chance + "%$se chance on hit to fire a $sdpiercing knife$se for $sd" + d(baseDamage) + "$se $ss(+" + d(stackDamage) + " per stack)$se base damage that gives $sh" + d(percentBarrierGain) + "$se plus an additional $sh" + flatBarrierGain + " barrier$se.").AutoFormat();
