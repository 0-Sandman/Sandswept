...
namespace Sandswept.Items.Whites
{
    ...
    public class AmberKnife : ItemBase<AmberKnife>
    {
        ...
        public override string ItemFullDescription => ("Gain a $sd" + chance + "%$se chance on hit to fire a $sdknife$se for $sd" + d(baseDamage) + "$se $ss(+" + d(stackDamage) + " per stack)$se base damage that $sdpierces$se and gives $sh" + flatBarrierGain + "$se, plus an additional $sh" + d(percentBarrierGain) + " barrier$se for each pierce.").AutoFormat();
